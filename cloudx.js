...
// 验证web组件的permission，能否被条件竞争绕过
async function checkPermission() { 
    try {
        if (window.testJsProxy) {
            let result = await window.testJsProxy.test();
            document.getElementById('userIdResult2').innerHTML += `<div class="suc">✅ demo test succeed<div class="result-content">${result}</div></div>`;
        } else {
            console.log('no window.testJsProxy')
            document.getElementById('userIdResult2').innerHTML += `<div class="err">❌ demo test error：no window.testJsProxy</div>`;
        }
    } catch (err) {
        console.error('demo test 执行异常:', err);
        document.getElementById('userIdResult2').innerHTML += `<div class="err">❌ demo test 执行异常：${err.message}</div>`;
    }
}
...
(async function runAllFunctions() {
    ...
    try { await checkPermission(); } catch (e) { console.error('执行demo test失败:', e); }
})();
